  ...
  ngOnInit(): void {
    ...
    if (usrAccsTkn!='null')
    {
      this.auth.getWithAuthorization(usrAccsTkn).subscribe(
        {
          next:() => {
            this.snkbr.openSnackBar('Phiên làm việc còn hạn','success');
            this.router.navigate(['main']);
          },
          error: () => {
              this.snkbr.openSnackBar('Phiên làm việc hết hạn','failed');
  ...
  onSubmit() {
    ...
    if (this.loginForm.valid) {
      this.auth.loginService(this.loginForm.value)
        .subscribe({
          next: (res) => {
            this.snkbr.openSnackBar('Đăng nhập thành công', 'success');
